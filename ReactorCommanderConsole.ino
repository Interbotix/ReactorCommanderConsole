...
//    Grip angle by right joystick horizontal while holding LT
...
// Global Objects
//=============================================================================
CommanderConsole command = CommanderConsole();
...
boolean ProcessUserInput3D(void) {
  ...
  if (g_bIKStatus == IKS_SUCCESS) {
    sIKX = min(max(g_sIKX + command.x_axis/10, IK_MIN_X), IK_MAX_X);
    sIKY = min(max(g_sIKY + command.y_axis/10, IK_MIN_Y), IK_MAX_Y);
    sIKZ = min(max(g_sIKZ + command.z_axis/15, IK_MIN_Z), IK_MAX_Z);
    sIKGA = min(max(g_sIKGA + command.w_angle/30, IK_MIN_GA), IK_MAX_GA);  // Currently in Servo coords...
    sGrip = min(max(sGrip + command.gripper/4, GRIP_MIN), GRIP_MAX);
    sWristRot = min(max(g_sWristRot + command.w_rot/4, WROT_MIN), WROT_MAX);
    }
    ...
    if (((g_sIKX > 0) && (command.x_axis < 0)) || ((g_sIKX < 0) && (command.x_axis > 0)))
      sIKX = min(max(g_sIKX + command.x_axis/10, IK_MIN_X), IK_MAX_X);
    if (((g_sIKY > 0) && (command.y_axis < 0)) || ((g_sIKY < 0) && (command.y_axis > 0)))
      sIKY = min(max(g_sIKY + command.y_axis/10, IK_MIN_Y), IK_MAX_Y);
    if (((g_sIKZ > 0) && (command.z_axis < 0)) || ((g_sIKZ < 0) && (command.z_axis > 0)))
      sIKZ = min(max(g_sIKZ + command.z_axis/15, IK_MIN_Z), IK_MAX_Z);
    if (((g_sIKGA > 0) && (command.w_angle < 0)) || ((g_sIKGA < 0) && (command.w_angle > 0))) 
      sIKGA = min(max(g_sIKGA, IK_MIN_GA), IK_MAX_GA);  // Currently in Servo coords...           
  }
  fChanged = (sIKX != g_sIKX) || (sIKY != g_sIKY) || (sIKZ != g_sIKZ) || (sIKGA != g_sIKGA || sGrip != g_sGrip) || (sWristRot != g_sWristRot) ;
...
boolean ProcessUserInputCylindrical() {
  ...
  sBase = min(max(g_sBase - command.x_axis/10, BASE_MIN), BASE_MAX);

  // Limit how far we can go by checking the status of the last move.  If we are in a warning or error
  // condition, don't allow the arm to move farther away...
  // Use Y for 2d distance from base
  if ((g_bIKStatus == IKS_SUCCESS) || ((g_sIKY > 0) && (command.y_axis < 0)) || ((g_sIKY < 0) && (command.y_axis > 0)))
    sIKY += command.y_axis/10;

  // Now Z coordinate...
  if ((g_bIKStatus == IKS_SUCCESS) || ((g_sIKZ > 0) && (command.z_axis < 0)) || ((g_sIKZ < 0) && (command.z_axis > 0)))
    sIKZ += command.z_axis/15;

  // And gripper angle.  May leave in Min/Max here for other reasons...   

  if ((g_bIKStatus == IKS_SUCCESS) || ((g_sIKGA > 0) && (command.w_angle < 0)) || ((g_sIKGA < 0) && (command.w_angle > 0)))
    sIKGA = min(max(g_sIKGA + command.w_angle/30, IK_MIN_GA), IK_MAX_GA);  // Currently in Servo coords...
    
    sGrip = min(max(sGrip + command.gripper/4, GRIP_MIN), GRIP_MAX);
    sWristRot = min(max(g_sWristRot + command.w_rot/4, WROT_MIN), WROT_MAX);    

  fChanged = (sBase != g_sBase) || (sIKY != g_sIKY) || (sIKZ != g_sIKZ) || (sIKGA != g_sIKGA || sGrip != g_sGrip) || (sWristRot != g_sWristRot) ;
...
boolean ProcessUserInputBackHoe() {
  ...
  // First the base
  boolean fChanged = false;
  sBase = min(max(g_sBase - command.x_axis/6, BASE_MIN), BASE_MAX);
  if (sBase != g_sBase)
    fChanged = true;

  // Now the Boom
  sShoulder = min(max(g_sShoulder + command.z_axis/6, SHOULDER_MIN), SHOULDER_MAX);
  if (sShoulder != g_sShoulder)
    fChanged = true;

  // Now the Dipper 
  sElbow = min(max(g_sElbow + command.y_axis/6, ELBOW_MIN), ELBOW_MAX);
  if (sElbow != g_sElbow)
    fChanged = true;

  // Bucket Curl
  sWrist = min(max(g_sWrist + command.w_angle/6, WRIST_MIN), WRIST_MAX);
  if (sWrist != g_sWrist)
    fChanged = true;

  //Wrist Rotate
  sWristRot = min(max(g_sWristRot + command.w_rot/4, WROT_MIN), WROT_MAX);
  if (sWristRot != g_sWristRot)
    fChanged = true;  
